<%-- 
    Document   : edit_video
    Created on : 27 Feb, 2022, 9:22:26 AM
    Author     : Genius
--%>
...
if(request.getParameter("code")==null &&request.getParameter("ccode")==null){
    response.sendRedirect("dashboard.jsp?coed_err=1");
}
else{
    ...
    if(email==null){
        ...
        try{
            ...
            ResultSet rs = st.executeQuery("select * from video where code='"+code+"'AND email='"+email+"'");
            if(rs.next()){
                %>
                <html>
      <head>
...
<div class="container-fluid">
  <div class="row" >
       ...
       <div class="col-sm-8 w3-card">
           <h3><strong><center>Edit Video Title & Description</center></strong></h3>
             <form method="post" action="update_video.jsp?code=<%=code%>&ccode=<%=ccode%>">
               <label><strong>Video Title:</strong></label>
               <input type="text" name="vname" value="<%=rs.getString("title")%>" class="form-control" required><br><br>
               <label><strong>Description:</strong></label>
			   <textarea name="des" class="form-control"><%=rs.getString("description")%></textarea><br><br>
  ...
  <div class="row">
             <div class="col-sm-2"></div>
    <div class="col-sm-8 w3-card">
        <h3><strong><center>Change Video Image</center></strong></h3>
                   <form method="post" ACTION="ChangeVideoImageUploadProcess.jsp?code=<%=code%>&ccode=<%=ccode%>" name="uploadForm" ENCTYPE='multipart/form-data'>
  ...
  <div class="row">
            <div class="col-sm-2"></div>
    <div class="col-sm-8 w3-card">
        <h3><strong><center>Change Video Mp4</center></strong></h3>
              <form method="post" ACTION="ChangeVideoUploadProcess.jsp?code=<%=code%>&ccode=<%=ccode%>" name="uploadForm" ENCTYPE='multipart/form-data'>
